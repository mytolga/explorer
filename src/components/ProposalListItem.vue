...
<template>
  <div class="bg-white dark:bg-[#181818] rounded text-sm">
        ...
        <tr v-for="(item, index) in proposals?.proposals" :key="index">
          <td class="px-4 w-20">
            <label
              ...
              class="text-main text-base hover:text-orange-400 cursor-pointer"
          ...
          <td class="w-full">
            <div>
              <RouterLink
                ...
                class="text-main text-base mb-1 block hover:text-orange-400 truncate"
              >
                {{ item?.content?.title || item?.title }}
              </RouterLink>
              <div
                v-if="item.content"
                class="bg-[#f6f2ff] text-[#ffa500] dark:bg-zinc-900 dark:text-gray-300 inline-block rounded-full px-2 py-[1px] text-xs mb-1"
              >
          ...
          <td v-if="statusMap?.[item?.status] === 'VOTING'" class="w-40">
            <div class="">
              <label
                for="vote"
                class="btn btn-xs btn-orange-300 rounded-sm"
                @click="
                  dialog.open('vote', {
    ...
    <div class="lg:!hidden">
      ...
      >
        <div
          class="text-main text-base mb-1 flex justify-between hover:text-orange-400"
        >
          <RouterLink
            :to="`/${chain.chainName}/gov/${item?.proposal_id}`"
        ...
        <div class="grid grid-cols-4 mt-2 mb-2">
          <div class="col-span-2">
            <div
              v-if="item.content"
              class="bg-[#f6f2ff] text-[#9c6cff] dark:bg-zinc-900 dark:text-gray-300 inline-block rounded-full px-2 py-[1px] text-xs mb-1"
            >
              {{ showType(item.content['@type']) }}
            </div>
          </div>
